...
function Sidebar({ items, name, active, onSelect }) {
  // Ajuste para mostrar solo hasta 2 líneas del nombre si es muy largo
  return (
    <aside className="w-56 shrink-0 bg-[#0F2038] rounded-2xl p-4 flex flex-col gap-1 h-full">
      ...
      {items.map(({ key, icon: Icon, label }) => (
        ...
        >
          {/* Escudo protector: Si el ícono no existe en tu versión, no colapsa */}
...
function TopBar({ points, name }) {
  // Extraemos el primer nombre (Asumiendo formato: Apellido1 Apellido2 Nombre1 Nombre2)
  const partesNombre = name.split(" ");
  const firstName = partesNombre.length > 2 ? partesNombre[2] : partesNombre[0];
...
function Login({ onEnter }) {
  ...
  const handleIngresar = (e) => {
    ...
    if (usuarioEncontrado) {
      ...
      // ¡Ahora pasamos los datos del usuario real!
      onEnter(role, usuarioEncontrado); 
    } else {
      setErrors({ 
        nombre: "No estás registrado o el nombre está mal escrito.", 
        dni: "El DNI ingresado no es válido para este usuario." 
      });
    }
  };
    ...
    <div className="grid grid-cols-3 gap-4 flex-1">
      <div className="col-span-2 bg-[#0F2038] rounded-2xl p-6">
        ...
        <div className="flex items-center gap-3">
          ...
          {books.map((b) => (
            <div key={b.title} className={`flex-1 rounded-xl p-5 text-center ${b.featured ? "bg-[#14284A] ring-2 ring-[#5AC8E8]" : "bg-[#14284A]"}`}>
        ...
        <div className="grid grid-cols-2 gap-4 mt-6">
          <div className="bg-[#14284A] rounded-xl p-5 flex items-center gap-3">
            <span className="text-4xl">🎮</span><span className="text-white text-sm font-bold">Juegos</span>
          </div>
          <div className="bg-[#14284A] rounded-xl p-5 flex items-center gap-3">
            <span className="text-4xl">🃏</span><span className="text-white text-sm font-bold">Flashcards</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#0F2038] rounded-2xl p-6">
        ...
        <div className="space-y-4">
          {notifications.map((n) => (
            <div key={n.title} className="flex gap-3">
              <div className="w-9 h-9 rounded-full flex items-center justify-center text-base shrink-0" style={{ backgroundColor: n.color + "33" }}>{n.emoji}</div>
              <div>
        ...
        <button className="w-full mt-5 bg-[#5AC8E8] text-[#0B1A33] text-xs font-bold py-2.5 rounded-lg">Ver calendario</button>
      </div>
    </div>
  );
}
...
function StudentDashboard({ onExit, user }) {
  const [active, setActive] = useState("inicio");
  // Si no hay usuario cargado, mostramos texto por defecto
  const nombreMostrar = user ? user.nombre : "Estudiante";

  return (
    <div className="flex gap-4 h-full">
      <Sidebar items={NAV_STUDENT} name={nombreMostrar} active={active} onSelect={setActive} />
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex justify-end mb-2">
          <button onClick={onExit} className="text-xs text-[#8FA3C0] hover:text-white underline">Cambiar de perfil</button>
        </div>
        <TopBar points="120" name={nombreMostrar} />
        {active === "inicio" ? <StudentHome /> : <SectionPlaceholder {...STUDENT_SECTIONS[active]} />}
      </div>
    </div>
  );
}
...
function TeacherHome() {
  const skills = [
    { label: "Pedagogía", value: 90 }, { label: "Comprensión Lectora", value: 75 },
    { label: "Evaluación Formativa", value: 60 }, { label: "TIC en Educación", value: 45 },
  ];
  const events = [
        ...
        <div className="bg-[#0F2038] rounded-2xl p-6">
          ...
          <div className="flex items-center gap-6">
            <svg width="110" height="110" viewBox="0 0 100 100" className="shrink-0">
              ...
              <circle cx="50" cy="50" r="42" stroke="#5AC8E8" strokeWidth="9" fill="none" strokeDasharray={circumference} strokeDashoffset={circumference * 0.28} strokeLinecap="round" transform="rotate(-90 50 50)" />
            ...
            <div className="flex-1 space-y-2.5">
              {skills.map((s) => (
                <div key={s.label}>
                  <div className="flex justify-between text-[10px] text-[#B7C4D9] mb-1"><span>✓ {s.label}</span></div>
                  <div className="h-1.5 bg-[#1E3660] rounded-full overflow-hidden"><div className="h-full bg-[#5AC8E8] rounded-full" style={{ width: `${s.value}%` }} /></div>
                </div>
              ))}
            </div>
          </div>
          <button className="w-full mt-5 bg-[#5AC8E8] text-[#0B1A33] text-xs font-bold py-2.5 rounded-lg">Continuar evaluación</button>
        </div>
        ...
        <div className="bg-[#0F2038] rounded-2xl p-6 flex flex-col">
          ...
          <div className="flex items-center gap-4 mt-5">
            ...
            <div className="flex-1">
              <div className="flex justify-between text-xs text-[#B7C4D9] mb-1"><span>950 / 1300 pts</span><span>75%</span></div>
              <div className="h-2 bg-[#1E3660] rounded-full overflow-hidden"><div className="h-full bg-[#5AC8E8] rounded-full" style={{ width: "75%" }} /></div>
            </div>
          </div>
          <p className="text-xs text-[#8FA3C0] mt-5"><span className="text-[#5AC8E8] font-semibold">Próximo nivel: </span>Completa las actividades para seguir avanzando.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 flex-1">
        <div className="bg-[#0F2038] rounded-2xl p-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center gap-2 text-xs text-[#8FA3C0]"><ChevronLeft size={14} /> Julio 2026 <ChevronRight size={14} /></div>
          </div>
          <div className="space-y-3">
            {events.map((e) => (
              <div key={e.title} className="flex gap-2 items-start">
                ...
                <div><div className="text-xs text-white font-medium">{e.title}</div><div className="text-[10px] text-[#8FA3C0]">{e.time}</div></div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {notes.map((n, i) => (
              <div key={i} className="flex justify-between items-center">
                <div className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: n.color }} /><span className="text-xs text-[#D6DFEC]">{n.text}</span></div>
                <span className="text-[10px] text-[#6E80A0]">{n.time}</span>
              </div>
            ))}
          </div>
          <button className="w-full mt-5 bg-[#5AC8E8] text-[#0B1A33] text-xs font-bold py-2.5 rounded-lg">Ver todas</button>
        </div>
      </div>
    </div>
  );
...
function TeacherDashboard({ onExit, user }) {
  const [active, setActive] = useState("inicio");
  const nombreMostrar = user ? user.nombre : "Docente";

  return (
    <div className="flex gap-4 h-full">
      <Sidebar items={NAV_TEACHER} name={nombreMostrar} active={active} onSelect={setActive} />
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex justify-end mb-2">
          <button onClick={onExit} className="text-xs text-[#8FA3C0] hover:text-white underline">Cambiar de perfil</button>
        </div>
        <TopBar points="950" name={nombreMostrar} />
        {active === "inicio" ? <TeacherHome /> : <SectionPlaceholder {...TEACHER_SECTIONS[active]} />}
      </div>
    </div>
  );
}
...
export default function NexoEduPrototype() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#0B1A33] flex items-center justify-center p-6 relative overflow-hidden">
      ...
      {/* Al ingresar, guardamos también los datos del usuario */}
      {view === "login" && <Login onEnter={(role, user) => { setView(role); setCurrentUser(user); }} />}
      
      {view === "estudiante" && (
        <div className="w-full h-[92vh] relative z-10">
          <StudentDashboard onExit={() => { setView("login"); setCurrentUser(null); }} user={currentUser} />
        </div>
      )}
      {view === "docente" && (
        <div className="w-full h-[92vh] relative z-10">
          <TeacherDashboard onExit={() => { setView("login"); setCurrentUser(null); }} user={currentUser} />
        </div>
      )}
    </div>
  );
